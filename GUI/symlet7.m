...
%Wavelet transform
[ca ch cv cd]=wt2(I,'sym7');
[m n]=size(ch);
[ca1 ch1 cv1 cd1]=wt2(ca,'sym7');
[m1 n1]=size(ch1);
[ca2 ch2 cv2 cd2]=wt2(ca1,'sym7');
[m2 n2]=size(ch2);
[ca3 ch3 cv3 cd3]=wt2(ca2,'sym7');
[m3 n3]=size(ch3);
[ca4 ch4 cv4 cd4]=wt2(ca3,'sym7');
[m4 n4]=size(ch4);
[ca5 ch5 cv5 cd5]=wt2(ca4,'sym7');
[m5 n5]=size(ch5);
[ca6 ch6 cv6 cd6]=wt2(ca5,'sym7');
[m6 n6]=size(ch6);
...
FM42_norm(img_count,1:22)=[(dc/1e+1) (b1/1e+4) (b2/1e+4) (b3/1e+3) (b4/1e+3) (b5/1e+3) (b6/1e+2) (b7/1e+2) (b8/1e+2) (b9/1e+1) (b10/1e+1) (b11/1e+1) (b12/1) (b13/1) (b14/1) (b15/1) (b16/1) (b17/1e-1) (b18/1) (b19/1e-2) (b20/1e-2) (b21/1e-2)];
FM42(img_count,1:22)=round(FM42_norm(img_count,1:22));
end
